...
void KeybJoystick::begin(bool generateVirtualKeys, bool createVKQueue, int PS2Port)
{
...
/***
void KeybJoystick::begin(gpio_num_t clkGPIO, gpio_num_t dataGPIO, bool generateVirtualKeys, bool createVKQueue)
{
  PS2Controller::begin(clkGPIO, dataGPIO);
  PS2Controller::setKeybJoystick(this);
  begin(generateVirtualKeys, createVKQueue, 0);
}
*/

void KeybJoystick::enableVirtualKeys(bool generateVirtualKeys, bool createVKQueue)
{
...
/**
  ...
  if (!m_SCodeToVKConverterTask && generateVirtualKeys)
    xTaskCreate(&SCodeToVKConverterTask, "", KeybJoystick::scancodeToVirtualKeyTaskStackSize, this, FABGLIB_SCODETOVK_TASK_PRIORITY, &m_SCodeToVKConverterTask);
...
bool KeybJoystick::setScancodeSet(int value)
{
  /**
...
bool KeybJoystick::setLEDs(bool numLock, bool capsLock, bool scrollLock)
{
...
void KeybJoystick::getLEDs(bool * numLock, bool * capsLock, bool * scrollLock)
{
...
int KeybJoystick::getNextScancode(int timeOutMS, bool requestResendOnTimeOut)
{
  /**
  while (true) {
...
/**
void KeybJoystick::setLayout(const KeyboardLayout * layout)
{
  m_layout = layout;
}
*/
...
char const * KeybJoystick::virtualKeyToString(VirtualKey virtualKey)
{
...
int KeybJoystick::virtualKeyToASCII(VirtualKey virtualKey)
{
  VirtualKeyItem item;
  item.vk         = virtualKey;
  item.down       = true;
...
/**
VirtualKey KeybJoystick::scancodeToVK(uint8_t scancode, bool isExtended, KeyboardLayout const * layout)
{
...
/**
VirtualKey KeybJoystick::VKtoAlternateVK(VirtualKey in_vk, bool down, KeyboardLayout const * layout)
{
  VirtualKey vk = VK_NONE;
...
void KeybJoystick::injectVirtualKey(VirtualKeyItem const & item, bool insert)
{
  // update m_VKMap
  if (item.down)
...
void KeybJoystick::injectVirtualKey(VirtualKey virtualKey, bool keyDown, bool insert)
{
  VirtualKeyItem item;
  item.vk          = virtualKey;
  item.down        = keyDown;
...
void KeybJoystick::postVirtualKeyItem(VirtualKeyItem const & item)
{
...
// -1 = no convertible
int KeybJoystick::convKeypadVKToNum(VirtualKey vk)
{
  switch (vk) {
    case VK_KP_0:
    case VK_KP_INSERT:
...
void KeybJoystick::SCodeToVKConverterTask(void * pvParameters)
{
  KeybJoystick * keyboard = (KeybJoystick*) pvParameters;
...
bool KeybJoystick::isVKDown(VirtualKey virtualKey)
{
...
bool KeybJoystick::getNextVirtualKey(VirtualKeyItem * item, int timeOutMS)
{
  bool r = false;/// (m_SCodeToVKConverterTask && item && xQueueReceive(m_virtualKeyQueue, item, msToTicks(timeOutMS)) == pdTRUE);
...
VirtualKey KeybJoystick::getNextVirtualKey(bool * keyDown, int timeOutMS)
{
  VirtualKeyItem item;
...
void KeybJoystick::convertScancode2to1(VirtualKeyItem * item)
{
...
uint8_t KeybJoystick::convScancodeSet2To1(uint8_t code)
{
